...
evaluation_results<- function(true, predicted, df) {
...
evaluation_results(y_train, predictions_train, training_data)

# Prediction and evaluation on test data
predictions_test <- predict(ridge_regression, s = optimal_lambda, newx = x_test)
evaluation_results(y_test, predictions_test, test_data)

#create csv
output= data.frame(Id = test_data$Id, income_morethan_50K = predictions_test)
write.csv(output, "ridge_reg.csv", row.names = FALSE, quote = F)
